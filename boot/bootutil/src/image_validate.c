                  ...
                  uint8_t *seed, int seed_len)
{
    ...
    do
    {
        ...
        {
            bootutil_sha_update(
                &sha_ctx, (void *)(IMAGE_RAM_BASE + hdr->ih_load_addr), size);
                break;
        }
 ...
 */
     ...
     defined(MCUBOOT_SIGN_XMSS)     + \
     defined(MCUBOOT_SIGN_MLDSA44)  + \
     defined(MCUBOOT_SIGN_MLDSA65)  + \
     defined(MCUBOOT_SIGN_MLDSA87)) > 1
...
#    define EXPECTED_SIG_LEN(x) (1) /*always true, LMS verify will check*/
...
#    define EXPECTED_SIG_TLV IMAGE_TLV_XMSS
#    define SIG_BUF_SIZE 2500  /* Approximate XMSS signature size */
#    define EXPECTED_SIG_LEN(x) ((x) >= 2400 && (x) <= 2600)  /* Allow range for XMSS */
#elif defined(MCUBOOT_SIGN_MLDSA44)
#    define EXPECTED_SIG_TLV IMAGE_TLV_MLDSA44
#    define SIG_BUF_SIZE 2500  /* ML-DSA-44 signature size ~2420 bytes */
#    define EXPECTED_SIG_LEN(x) ((x) >= 2400 && (x) <= 2500)  /* Allow range for ML-DSA-44 */
#elif defined(MCUBOOT_SIGN_MLDSA65)
#    define EXPECTED_SIG_TLV IMAGE_TLV_MLDSA65
#    define SIG_BUF_SIZE 3400  /* ML-DSA-65 signature size ~3309 bytes */
#    define EXPECTED_SIG_LEN(x) ((x) >= 3300 && (x) <= 3400)  /* Allow range for ML-DSA-65 */
#elif defined(MCUBOOT_SIGN_MLDSA87)
#    define EXPECTED_SIG_TLV IMAGE_TLV_MLDSA87
#    define SIG_BUF_SIZE 4700  /* ML-DSA-87 signature size ~4627 bytes */
#    define EXPECTED_SIG_LEN(x) ((x) >= 4600 && (x) <= 4700)  /* Allow range for ML-DSA-87 */
#else
#    define SIG_BUF_SIZE 32 /* no signing, sha256 digest only */
#endif

/* Complex result masks for bootutil_img_validate() */
#define SET_MASK_IMAGE_TLV_SHA       ((signed)0x0000002E)
fih_int FIH_MASK_IMAGE_TLV_SHA = FIH_INT_INIT_GLOBAL(
        SET_MASK_IMAGE_TLV_SHA);
...
#define CHK_MASK_IMAGE_TLV_SHA       SET_MASK_IMAGE_TLV_SHA
    ...
    defined(MCUBOOT_SIGN_EC521) || \
    defined(MCUBOOT_SIGN_LMS)   || \
    defined(MCUBOOT_SIGN_ED25519) || \
    defined(MCUBOOT_SIGN_XMSS)  || \
    defined(MCUBOOT_SIGN_MLDSA44) || \
    defined(MCUBOOT_SIGN_MLDSA65) || \
    defined(MCUBOOT_SIGN_MLDSA87)
...
bootutil_find_key(uint8_t *keyhash, uint8_t keyhash_len)
{
    ...
    uint8_t hash[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];

    if (keyhash_len != BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE) {
        return -1;
    }
    ...
    for (i = 0; i < bootutil_key_cnt; i++) {
        ...
        /* SFLASH stores RAW key (0x04 + X + Y), but edgeprotecttools hashes DER format
        * So bootutil_find_key must convert RAW to DER before hashing */
        uint8_t der_key[EC_KEY_SZ]; /* Buffer for DER-encoded key */
        size_t der_len = 0;
        int rc;
        
        /* Convert RAW key to DER using MbedTLS ASN.1 functions */
        rc = build_der_key_with_mbedtls(der_key, &der_len, key->key, *key->len);
        if (rc != 0) {
            BOOT_LOG_ERR("Failed to build DER key: rc=%d", rc);
            bootutil_sha_drop(&sha_ctx);
            return -1;
        }
        ...
        bootutil_sha_init(&sha_ctx);
        bootutil_sha_update(&sha_ctx, der_key, der_len);
        bootutil_sha_finish(&sha_ctx, hash);

#else
        /* Standard path: hash the key as-is */
        bootutil_sha_init(&sha_ctx);
...
bootutil_find_key(uint8_t image_index, uint8_t *key, uint16_t key_len)
{
    bootutil_sha_context sha_ctx;
    uint8_t hash[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    uint8_t key_hash[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
                      ...
                      int seed_len, uint8_t *out_hash)
{
    ...
    if (out_hash) {
        (void)memcpy(out_hash, hash, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
    }
    ...
    while (true) {
         ...
         * unprotected area. All other TLVs must be in the protected section.
         */
        if (!bootutil_tlv_iter_is_prot(&it, off)) {
            ...
            if (!found) {
                BOOT_LOG_DBG("TLV type 0x%x not permitted in unprotected area", type);
                rc = -1;
                goto out;
            }
        }

        if (type == IMAGE_TLV_SHA) {
            /*
            ...
            if (fih_eq(fih_rc, FIH_SUCCESS)) {
                ...
                fih_complex_result = fih_or(fih_complex_result,
                                                 FIH_MASK_IMAGE_TLV_SHA);
            }
            else {
                BOOT_LOG_DBG("IMAGE_TLV_SHA is invalid");
                rc = -1;
                goto out;
            }
             ...
             */
            if (len != BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE) {
                rc = -1;
                goto out;
            }
