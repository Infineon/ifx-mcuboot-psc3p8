        ...
        const uint8_t *salt, uint16_t salt_len, uint8_t *okm, uint16_t *okm_len)
{
    ...
    uint8_t prk[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    uint8_t T[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    uint16_t off;
    uint16_t len;
    uint8_t counter;
    bool first;
    ...
    rc = bootutil_hmac_sha256_finish(&hmac, prk, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
    if (rc != 0) {
        goto error;
    }
    ...
    counter = 1;
    first = true;
    for (off = 0; len > 0; off += BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE, ++counter) {
        bootutil_hmac_sha256_init(&hmac);

        rc = bootutil_hmac_sha256_set_key(&hmac, prk, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
        if (rc != 0) {
            goto error;
        }
        ...
        if (first) {
            ...
            rc = bootutil_hmac_sha256_update(&hmac, T, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
            if (rc != 0) {
                goto error;
            }
        }
        ...
        rc = bootutil_hmac_sha256_finish(&hmac, T, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
        if (rc != 0) {
            goto error;
        }

        if (len > BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE) {
            (void)memcpy(&okm[off], T, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
            len -= BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE;
        } else {
...
#    define EXPECTED_ENC_EXT_LEN (EXPECTED_ENC_LEN + BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE)
...
boot_enc_decrypt(const uint8_t *buf, uint8_t *enckey, uint32_t sz, uint8_t *enciv)
{
    ...
    uint8_t salt[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    uint8_t tag[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    uint8_t info[BOOTUTIL_CRYPTO_AES_CTR_BLOCK_SIZE] = "MCUBoot_ECIES_v1";
    uint8_t shared[SHARED_KEY_LEN];
    uint8_t derived_key[BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE + BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE + BOOTUTIL_CRYPTO_AES_CTR_BLOCK_SIZE * 2];
    uint8_t *cp;
    uint8_t *cpend;
    ...
    (void)memset(enciv, 0, BOOTUTIL_CRYPTO_AES_CTR_BLOCK_SIZE);
    ...
    (void)memset(salt, 0, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);

    len = BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE + BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE;
    ...
    if (sz > EXPECTED_ENC_LEN) {
        ...
        my_key_iv = &derived_key[BOOTUTIL_CRYPTO_AES_CTR_KEY_SIZE +
                                 BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    ...
    rc = hkdf(shared, SHARED_KEY_LEN, info, BOOTUTIL_CRYPTO_AES_CTR_BLOCK_SIZE,
              my_salt, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE, derived_key, &out_len);
    ...
    rc = bootutil_hmac_sha256_set_key(&hmac, &derived_key[BOOT_ENC_KEY_SIZE], BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
    if (rc != 0) {
    ...
    rc = bootutil_hmac_sha256_finish(&hmac, tag, BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE);
    if (rc != 0) {
        (void)bootutil_hmac_sha256_drop(&hmac);
        return -1;
    }

    if (bootutil_constant_time_compare(tag, &buf[EC_TAG_INDEX], BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE) != 0) {
        (void)bootutil_hmac_sha256_drop(&hmac);
        return -1;
    }
