                   ...
                   struct boot_status *bs)
{
    ...
    // Quick sanity check before opening flash, but allow erased headers through
    // to reach the trailer cleanup logic below (lines 864-881)
    if (hdr->ih_magic != IMAGE_MAGIC && boot_check_header_erased(state, slot) != 0) {
        BOOT_LOG_DBG("Invalid magic in slot %d (0x%x), validation fail", slot, hdr->ih_magic);
        FIH_RET(FIH_FAILURE);
    }
...
{
    return boot_read_slot_flag(image_id, slot_id, &boot_read_image_inactive);
}

int
boot_is_slot_booted(uint32_t image_id, uint32_t slot_id)
{
    return boot_read_slot_flag(image_id, slot_id, &boot_read_copy_done);
}

int
boot_is_slot_confirmed(uint32_t image_id, uint32_t slot_id)
{
    return boot_read_slot_flag(image_id, slot_id, &boot_read_image_ok);
}
