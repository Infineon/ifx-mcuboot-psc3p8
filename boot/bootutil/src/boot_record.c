...
#if defined(PSC3_P8)
            BOOT_LOG_WRN("Shared memory is already added.");
                      ...
                      const struct flash_area *fap)
{
    ...
    uint16_t ias_minor;
    size_t record_len = 0;
    uint8_t image_hash[BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE];
    ...
    while (true) {
        ...
        } else if ((uint16_t)IMAGE_TLV_SHA == type) {
            /* Get the image's hash value from the manifest section. */
            if (len != (uint16_t)BOOTUTIL_CRYPTO_SHA_DIGEST_SIZE) {
                return -1;
            }
