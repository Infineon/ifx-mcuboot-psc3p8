...
class MemoryMap:
    ...
    def __init__(self):
        ...
        self.generate_app_configs                       = None
        self.max_sectors                                = 32
    ...
    def parse(self, memory_map, platform_config, output_folder, output_name, generate_app_configs, app_id):
        ''''
        ...
        try:
            ...
            self.generate_app_configs  = generate_app_configs
            ...
            if app_id is None:
                ...
                if self.generate_app_configs is not None:
                    print('\n[memorymap_rework.py] Generating app boot sources', file=sys.stderr)
                    self.__source_gen()
                    self.__header_gen()
...
def cli():
    ...
    parser.add_argument('-g', '--generate_app_configs', required=False,
                        help='generate memorymap source and header files for application')
    ...
    run(args.memory_config, args.platform_config, args.output_folder, args.output_name,
        args.generate_app_configs, args.image_id)


def run(memory_config, platform_config, output_folder, output_name, generate_app_configs, image_id):
    ''''
