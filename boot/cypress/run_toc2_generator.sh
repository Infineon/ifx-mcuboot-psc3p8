...
if [ "$LCS" == "NORMAL_NO_SECURE" ]; then
    ...
    if [ "$ENC_OPTION" == "1" ]; then
		echo "Error: Unsupported configuration." > /dev/tty

		# Create the binary file and fill it with 12 random bytes
		#dd if=/dev/random of=$AES_CTR_NONCE_FILE bs=1 count=12 conv=notrunc
